...
export const initPictureFormValidation = ()=>{
  ...
  }, 'Хэш-тег должен начинаться с символа #', 6,true);
  pristine.addValidator(inputHashtag, (value) => {
    const hashtags = value.trim().split(/\s+/).filter(Boolean);
    return hashtags.every((tag) => tag.length > 1);
  }, 'Хэш-тег не может состоять только из одной решётки',5,true);
  pristine.addValidator(inputHashtag, (value) => {
    const hashtags = value.trim().split(/\s+/).filter(Boolean);
    const hashtagRegex = /^#[a-zA-ZА-ЯЁа-яё0-9]+$/;
    return hashtags.every((tag) => hashtagRegex.test(tag));
  }, 'Строка после решётки должна состоять из букв и чисел и не может содержать пробелы, спецсимволы, символы пунктуации и т. д.',4,true);
  pristine.addValidator(inputHashtag, (value) => {
    const hashtags = value.trim().split(/\s+/).filter(Boolean);
    return hashtags.every((tag) => tag.length <= 20);
  }, 'Максимальная длина одного хэш-тега 20 символов, включая решётку',3,true);
  pristine.addValidator(inputHashtag, (value) => {
    const hashtags = value.trim().split(/\s+/).filter(Boolean);
    const containsSpaces = value.trim().includes(' ');
    return (hashtags.length <= 1 || containsSpaces);
  }, 'Хэш-теги разделяются пробелами',2,true);
